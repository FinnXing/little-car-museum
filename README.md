...
> 当前阶段：TASK-005 汽车详情页。已接入独立 3D 查看器，支持儿童版介绍、收藏、热点、素材信息和通用模型声明；模型仍为馆内自制通用占位内容。
...
TASK-003 已提供 `/cars` 汽车展厅、分类筛选、响应式汽车卡片和完整页面状态。TASK-004 新增 `/prototype/viewer` 独立观察台，支持模型加载与图片降级。TASK-005 新增 `/cars/[slug]` 详情页，包含查看器、收藏、热点和素材信息。由于占位车辆均为草稿，它们只会在 `next dev` 中显示；生产构建默认进入空状态，避免公开未发布内容。

下一步为 TASK-006：本地收藏和最近浏览。完整顺序和任务范围见需求文档第 35 章，不自动扩展到后续任务。
