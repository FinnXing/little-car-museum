...
> 当前阶段：TASK-004 独立 3D 查看器原型。已实现本地 GLB 加载、进度反馈、旋转/缩放、视角预设、自动旋转、颜色切换、资源释放及图片降级；模型仍为馆内自制通用占位内容。
...
TASK-003 已提供 `/cars` 汽车展厅、分类筛选、响应式汽车卡片和完整页面状态。TASK-004 新增 `/prototype/viewer` 独立观察台，支持模型加载与图片降级。由于占位车辆均为草稿，它们只会在 `next dev` 中显示；生产构建默认进入空状态，避免公开未发布内容。汽车卡片暂不跳转到尚未实现的详情页，收藏仅展示“未收藏”状态。

下一步为 TASK-005：汽车详情页与导航接入。完整顺序和任务范围见需求文档第 35 章，不自动扩展到后续任务。
