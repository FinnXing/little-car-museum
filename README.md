...
> 当前阶段：TASK-003 汽车列表。已实现开发环境列表预览、分类筛选及加载/空数据/失败状态；尚未开发 3D、汽车详情、数据库或后台业务。
...
TASK-003 已提供 `/cars` 汽车展厅、分类筛选、响应式汽车卡片和完整页面状态。由于占位车辆均为草稿，它们只会在 `next dev` 中显示；生产构建默认进入空状态，避免公开未发布内容。汽车卡片暂不跳转到尚未实现的详情页，收藏仅展示“未收藏”状态。

下一步为 TASK-004：独立 3D 查看器原型。完整顺序和任务范围见需求文档第 35 章，不自动扩展到后续任务。
