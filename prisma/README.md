...
本目录维护 PostgreSQL 数据模型、迁移和开发环境 Seed。Seed 只写入明确标记为 `DRAFT` 的通用占位汽车，不会形成公开内容，也不替代正式素材授权审核。上传文件的对象 key、摘要和上传管理员记录在 `AssetObject` 表中，实际二进制内容由 `STORAGE_DRIVER` 指定的对象存储保存。
